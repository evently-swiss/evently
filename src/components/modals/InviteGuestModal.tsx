...
type InviteGuestModalProps = {
  eventId: string;
  inviteGuestAction: (eventId: string, prevState: ActionState, formData: FormData) => Promise<ActionState>;
};

function InviteGuestForm({ eventId, inviteGuestAction, onSuccess }: InviteGuestModalProps & { onSuccess: () => void }) {
  const action = inviteGuestAction.bind(null, eventId);
  ...
  return (
    <form action={dispatch} className="space-y-4">
      ...
      {state?.message && (
        <p className={`${state.success ? 'text-green-400' : 'text-red-500'} text-sm mt-2`}>{state.message}</p>
      )}
    </form>
  );
}
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Creates a pending RSVP and sends a link to confirm or decline.
                  </p>
                  <div className="mt-2">
                    <InviteGuestForm key={version} {...props} onSuccess={() => setIsOpen(false)} />
                  </div>
                </div>
              </div>
            </div>
